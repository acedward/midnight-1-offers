...
# fees. Divergences are marked DIVERGENCE and justified where they occur; there are five.
#
# The fifth is new in this 1.x repository and it is SETTLEMENT-CRITICAL, not cosmetic: the block
# cadence. See CELESTIA_BLOCK_TIME below.
#
# ─── PERSISTENCE ─────────────────────────────────────────────────────────────────────────────
...
# every restart is not what the offer-files kernel meets in this stack. `./down.sh -v` wipes it,
# and it must be wiped together with the Midnight volumes, since offer state spans both chains.
#
# ─── HEALTH ──────────────────────────────────────────────────────────────────────────────────
...
# the sibling 2.x repository's umbra-evm WebSocket server before its upstream fix. On a developer
# laptop the default is correct; in a container it is unusable, and the failure mode (connection
# refused through a port that probes as open) names nothing.
...
# DIVERGENCE 5 (block cadence). `@effectstream/celestia@0.103.1` hardcodes
# `--delayed-precommit-timeout 1s`, i.e. roughly one Celestia block per second. This devnet runs
# at 3 s instead, which is the cadence the kernel repository's OWN acceptance harness pins
# (`packages/tests/grand-e2e/lib/solver-offerfiles-real-e1.ts`: `celestiaBlockTime: "3s"`,
# described there as "matching current Celestia mainnet"). The same source records that
# `timeout_commit` in config.toml is DEPRECATED AND INERT on celestia-app 6.4.10, so this flag
# is the only control that actually moves the cadence.
#
# WHAT THE 1s CADENCE COSTS, STATED ACCURATELY FOR THIS KERNEL LINE. On the kernel branch the
# 00005-E1 e2e ran against, a ~1 s cadence left the kernel's Celestia projection permanently
# more than `MAX_CELESTIA_LAG_BLOCKS = 4` behind the DA head, and every read gated on
# `requireCurrentBackend` answered 503 forever — measured, 60/60 exact-files reads.
#
# THAT GATE DOES NOT EXIST ON KERNEL `main` (verified at the pinned commit: no
# `MAX_CELESTIA_LAG_BLOCKS`, no `requireCurrentBackend`, no `deriveSyncStatus` anywhere, and no
# `/v1/offers/files` route to be refused). So on main a 1 s cadence does not produce that 503 —
# it produces the same underlying condition with NO error at all: a projection running a large
# permanent deficit, visible only as `celestia.lag_blocks` in `GET /v1/health/sync`. That is a
# worse failure mode, not a better one, and it is also what the solver's mirror reads from.
#
# So 3 s stays, for three reasons that survive the missing gate: it is the kernel repository's
# own tested cadence, it keeps the projection close enough for the lag this stack asserts in
# scripts/verify-kernel.sh, and the solver profile is built from a kernel line that DOES
# reinstate the gate.
#
# Its partner is CELESTIA_POLLING_INTERVAL_MS=1000 on the kernel side (main's own devnet default
# is 6000 ms — one poll per six blocks at the packaged cadence, which reintroduces the same
# deficit as a sawtooth). Both defaults live in .env.example; neither is a tuning knob.
BLOCK_TIME="${CELESTIA_BLOCK_TIME:-3s}"
...
start_app() {
  local args=(
    ...
    --rpc.unsafe
    --grpc-web.enable
    # DIVERGENCE 5, see BLOCK_TIME above: 3s, not the packaged 1s. At 1s the kernel's projection
    # never gets inside MAX_CELESTIA_LAG_BLOCKS and POST /v1/offers/files is permanently 503.
    --delayed-precommit-timeout "$BLOCK_TIME"
  )
...
start_bridge() {
  ...
  # token path is the one the offer-files services wire up and verify.sh proves.
# CELESTIA_SKIP_AUTH=true restores the
...
# Invoked only through the INT/TERM trap installed below.
...
say "chain-id=${CHAIN_ID} namespace=${CELESTIA_NAMESPACE:-<unset>} block-time=${BLOCK_TIME} auth=$( [[ "$SKIP_AUTH" == "true" || "$SKIP_AUTH" == "1" ]] && echo skipped || echo required )"
