...
# stack must never hand them: here it is the faucet, the offer-files deploy/mint wallet AND the
# kernel's MIDNIGHT_WALLET_SEED. Two wallet facades on one seed force each other's connection
# down, silently (wallets/wallets.json).
...
refuse_genesis_1() {
  ...
  if [ "${seed}" = "${GENESIS_1_SEED}" ]; then
    ...
    log "In this stack that seed is the faucet, the offer-files deploy/mint wallet and the"
    log "kernel's MIDNIGHT_WALLET_SEED. A second facade on it takes one of them offline with"
    log "no error naming the cause. See wallets/wallets.json and spec FR-009."
    exit 78
  fi
}
