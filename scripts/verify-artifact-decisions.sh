...
# "does this repository still make the artifact choices it promised?" — so a later change
# cannot quietly repack a good official image, fall back to a source build where an exact
# warehouse binary exists, pin by tag instead of digest, drop a Linux platform, record an
# attestation manifest as a runnable one, install a macOS asset into a Linux container, pin
# a source build to a branch, hand the PRIVATE relay source a fetchable URL, or half-migrate
# the proof-data strategy.
#
...
#   ./scripts/verify-artifact-decisions.sh --update-pins # after a re-verified identity edit
set -euo pipefail
