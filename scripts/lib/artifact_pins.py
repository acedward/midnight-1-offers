...
this repository. Shell scripts that need one of those values must read it from there rather
than keep a second copy that can drift — which is exactly the failure the matrix exists to
prevent.

    scripts/lib/artifact_pins.py config/artifact-decisions.json \\
        'components[midnight-node].oci.indexDigest'
    scripts/lib/artifact_pins.py config/artifact-decisions.json 'sources[cow-solver].ref'
...
resolve. `--optional` turns a missing path or a null value into exit 0 with empty output,
for fields the record truthfully leaves null (a `legacy-unverified` member hash, or the
proof-data generation while the pre-warm strategy is in force).
