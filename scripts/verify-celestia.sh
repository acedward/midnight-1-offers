...
#                  the exact mechanism the offer-files kernel uses, so it is checked as a mechanism and
...
#                  here is what makes the kernel's own wiring routine rather than a discovery.
...
# loopback-bound listener (the published-port proxy trap documented in scripts/lib/celestia.sh).
#
set -euo pipefail
...
if (( AUTH_REQUIRED )); then
  ...
  # entrypoint-common.sh depends on: `set -a; . /celestia/auth/celestia.env; set +a`.
